...
public:
  ...
  static quaternion ji(){return -1*k();}
  static quaternion kj(){return -1*i();}
  static quaternion ik(){return -1*j();}
  ...
  friend quaternion operator+(const quaternion& q, T k){return quaternion(k, q.x + k, q.y + k, q.z + k);}
  friend quaternion operator+(T k, const quaternion& q){return quaternion(k, q.x + k, q.y + k, q.z + k);}


  friend quaternion operator-(const quaternion& q, T k){return quaternion(-1*k, q.x - (-1*k), q.y - (-1*k), q.z - (-1*k));}
  friend quaternion operator-(T k, const quaternion& q){return (-1*quaternion(k, q.x - k, q.y - k, q.z - k));}
  ...
  quaternion operator-() const{return quaternion(-1*this->w, -1*this->x, -1*this->y, -1*this->z);}

  friend bool operator==(const quaternion& q, const quaternion& r){
    if((q.w==r.w)&&(q.x==r.x)&&(q.y==r.y)&&(q.z==r.z)){
      return true;
    }
    else return false;
  }
  ...
  quaternion conjugate() const{return quaternion(this->w, -1*this->x, -1*this->y, -1*this->z);}
  ...
  matrix3d<T> rot_matrix() const{
    ...
    return matrix3d<T>("rot_matrix", 3, 
                        { -2*(pow(y,2)+pow(z,2))+1, 2*((x*y)-(w*z)), 2*((x*z)+(w*y)),
                          2*((x*y)+(w*z)), -2*(pow(x,2) + pow(z,2))+1, 2*((y*z)-(w*x)),
                          2*((x*z)-(w*y)), 2*((y*z)+(w*x)), -2*(pow(x,2) + pow(y,2))+1 });
  }
 ...
 static vec3 rotate(const vector3D& pt, const vector3D& axis, double theta){
   double costhetha2 = cos(theta/2.0);
   double sintheta2 = sin(theta/2.0);
   quaternion q = quaternion(costhetha2, axis.x()*sintheta2, axis.y()*sintheta2, axis.z()*sintheta2);
